...
<div class="bottom30 navBarFlitro row">
	<nav class="bottom30i navbar navbar-expand-lg navbar-darck bg-secondary"> <!-- nav-gradiente -->
	...
	<div class="collapse navbar-collapse" id="navbarTogglerDemo01">
		...
		<form id="filtro"  class="form-inline form-row mx-auto " action="<?php echo base_url('/filtro')?>" method="GET"> 
				<div class="col-12 text-center text-white">
					<a class="text-white">Seleccionar filtros de búsqueda para proveedor:</a>
				</div>
				<div class="col-10">
					<div class="row text-center">
						<div class="col-3 col-sm-3">
							<div class="form-group select">
								<select name="region" id="regionPRO" data-live-search="true" class="form-control select-lg selectpicker"  style="width:100% !important;">
						...
						<div class="col-3 col-sm-3">
							<div class="form-group select">
								<select name="comuna" id="comunaPRO" data-live-search="true" class="form-control select-lg selectpicker"  style="width:100% !important;">
						...
						<div class="col-3 col-sm-3">
							<div class="form-group select">
								<select name="categoriaSelect" id="categoriaSelectPro" class="selectpicker select-lg" title="Categoría(s)" 
						...
						<div class="col-3 col-sm-3">
							<div class="form-group select">
								<select name="subCategoriaSelect" id="subCategoriaSelectPro" class="selectpicker select-lg" title="Subcategoría(s)" 
								multiple data-live-search="true" style="width:100% !important;">
									<option >-Subcategoría-</option>
									<?php foreach($subCategoria as $row):?>
										<option value="<?= $row['idSubCat'];?>"><?= $row['nombreSub'];?></option>
									<?php endforeach;?>
									<input type="hidden" class="form-control form-control-user" id="listSubCategoriasPro" name="listSubCategorias">
								</select>
							</div>
						</div>
					</div>
...
<script>
    $("#regionPRO").change(function() {
       		$("#regionPRO option:selected").each(function() {
            	...
            	var id = $('#regionPRO').val();
            	var select = document.getElementById("comunaPRO"); //Seleccionamos el select
        	...
        	$('#comunaPRO').selectpicker('refresh');
    });
...
<script>
    ...
    $('#subCategoriaSelectPro').on('change', function(){
        ...
        //alert(subCat);
		$('#listSubCategoriasPro').val(subCat);
    });
    $('#categoriaSelectPro').on('change', function(){
        var selected = $(this).find("option:selected");
        var arrSelected = [];
		var  cat = "";
        selected.each(function(){
		...
		$select = document.getElementById("subCategoriaSelectPro");
		...
		$('#subCategoriaSelectPro').append(optgroup);
        $('#subCategoriaSelectPro').selectpicker('refresh');
